...
double left_kp = 5.9 , left_ki = 0.0 , left_kd = 0.0;             // modify for optimal performance
double right_kp = 5.9 , right_ki = 0.0 , right_kd = 0.0;
...
void loop() {
  ...
  if (currentMillis - prevMillis >= LOOPTIME){
    ...
    demand_speed_left = demandx - (demandz*0.167);
    demand_speed_right = demandx + (demandz*0.167);
    ...
    encoder0Error = (demand_speed_left*90.0)-encoder0Diff; // 6340 ticks in 1m, 90.00 ticks in 10ms due to the 10 millis loop
    encoder1Error = (demand_speed_right*90.0)-encoder1Diff;
  
    encoder0Prev = encoder0Pos; // Saving values
    encoder1Prev = encoder1Pos;
  
    left_setpoint = demand_speed_left*90.0;  //Setting required speed as a mul/frac of 1 m/s
